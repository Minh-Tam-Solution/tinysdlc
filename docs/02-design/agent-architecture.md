...
<!-- Originally: docs/AGENTS.md -->

TinySDLC supports running multiple AI agents simultaneously, each with its own isolated workspace, configuration, and conversation state. This allows you to have specialized agents for different tasks while maintaining complete isolation.
...
// Result: { agentId: "coder", message: "fix the authentication bug", isTeam?: boolean }
```
...
If no agents are configured, TinySDLC automatically creates a default agent using the legacy `models` section:
...
For detailed troubleshooting of agent-related issues, see [Troubleshooting Guide](../04-build/troubleshooting-guide.md).
...
tinysdlc team add                 # Add a new team (interactive)
...
- Setup wizard: `tinysdlc setup`
- Agent CLI: `tinysdlc agent --help`
