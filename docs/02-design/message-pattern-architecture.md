...
<!-- Originally: docs/MESSAGE-PATTERNS.md -->

Team communication in TinySDLC uses an actor model: each agent has its own mailbox (per-agent promise chain), communicates only by sending messages (queue files), and processes one message at a time. There is no central orchestrator.
...
`totalMessages` is capped at 50 (`MAX_CONVERSATION_MESSAGES`). When reached, no further mentions are enqueued and active branches resolve naturally.
