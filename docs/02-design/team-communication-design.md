...
For detailed message patterns (fan-out, backflow, cross-talk, shared context), see [Message Pattern Architecture](message-pattern-architecture.md).
...
See [Message Pattern Architecture](message-pattern-architecture.md) for detailed documentation on:
...
```bash
tinysdlc team visualize         # Watch all teams
tinysdlc team visualize dev     # Watch specific team
```
...
```bash
tinysdlc team list              # List all teams
tinysdlc team add               # Add a new team (interactive wizard)
tinysdlc team show dev          # Show team configuration
tinysdlc team remove dev        # Remove a team
tinysdlc team visualize [id]    # Live TUI dashboard
```
...
```bash
# 1. Create agents
tinysdlc agent add    # Create "coder" agent
tinysdlc agent add    # Create "reviewer" agent
...
- [Message Pattern Architecture](message-pattern-architecture.md) - Message flow patterns, shared context, pending indicators
- [Agent Architecture](agent-architecture.md) - Agent configuration and management
- [Queue System Design](queue-system-design.md) - Queue system and message processing
