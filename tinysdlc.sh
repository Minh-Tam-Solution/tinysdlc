#!/usr/bin/env bash
# TinySDLC - Main daemon using tmux + claude -c -p + messaging channels
#
...
TMUX_SESSION="tinysdlc"
LOG_DIR="$TINYSDLC_HOME/logs"
SETTINGS_FILE="$TINYSDLC_HOME/settings.json"
...
case "${1:-}" in
            ...
            case "$PROVIDER_ARG" in
                anthropic)
                    ...
                    if [ -n "$MODEL_ARG" ]; then
                        ...
                        echo ""
                        echo "Use 'tinysdlc model {sonnet|opus}' to set the model."
                    fi
                    ;;
                openai)
                    if [ ! -f "$SETTINGS_FILE" ]; then
                    ...
                    if [ -n "$MODEL_ARG" ]; then
                        ...
                        echo ""
                        echo "Use 'tinysdlc model {gpt-5.3-codex|gpt-5.2}' to set the model."
                        echo "Note: Make sure you have the 'codex' CLI installed and authenticated."
                    fi
                    ;;
                *)
    ...
    *)
        local_names=$(IFS='|'; echo "${ALL_CHANNELS[*]}")
        echo -e "${BLUE}TinySDLC - Claude Code + Messaging Channels${NC}"
        echo ""
        echo "Usage: $0 {start|stop|restart|status|setup|send|logs|reset <agent_id>|channels|provider|model|agent|team|pairing|sdlc|update|attach}"
        echo ""
        echo "Commands:"
        echo "  start                    Start TinySDLC"
        echo "  stop                     Stop all processes"
        echo "  restart                  Restart TinySDLC"
        ...
        echo "  sdlc {status|roles|init}  SDLC Framework v6.0.6 commands"
