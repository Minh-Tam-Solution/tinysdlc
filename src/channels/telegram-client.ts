#!/usr/bin/env node
/**
 * Telegram Client for TinySDLC Simple
...
function downloadFile(url: string, destPath: string, _redirectDepth = 0): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        ...
        function handleResponse(response: http.IncomingMessage): void {
            ...
            // SEC-010: reject oversized files early via Content-Length header
            const contentLength = parseInt(response.headers['content-length'] || '0', 10);
            if (contentLength > MAX_DOWNLOAD_BYTES) {
                response.destroy();
                file.close();
                fs.unlink(destPath, () => {});
                reject(new Error(`File too large: ${contentLength} bytes (limit: ${MAX_DOWNLOAD_BYTES})`));
                return;
            }

            // SEC-010: enforce limit while streaming
            let received = 0;
...
bot.getMe().then(async (me: TelegramBot.User) => {
    ...
    await bot.setMyCommands([
        ...
        { command: 'workspace', description: 'Manage project workspaces' },
