#!/usr/bin/env node
/**
 * WhatsApp Client for TinySDLC Simple
...
const LOG_FILE = path.join(TINYSDLC_HOME, 'logs/whatsapp.log');
const SESSION_DIR = path.join(SCRIPT_DIR, '.tinysdlc/whatsapp-session');
const SETTINGS_FILE = path.join(TINYSDLC_HOME, 'settings.json');
const FILES_DIR = path.join(TINYSDLC_HOME, 'files');
...
const MAX_DOWNLOAD_BYTES = 50 * 1024 * 1024; // 50 MB — SEC-010
...
async function downloadWhatsAppMedia(message: Message, queueMessageId: string): Promise<string | null> {
    try {
        ...
        // SEC-010: reject oversized files before writing to disk
        const byteLength = Buffer.byteLength(media.data, 'base64');
        if (byteLength > MAX_DOWNLOAD_BYTES) {
            log('WARN', `[SEC] WhatsApp media rejected — too large: ${byteLength} bytes`);
            return null;
        }
...
setInterval(() => {
    ...
    try {
        ...
        for (const status of statuses) {
            ...
            pending.chat.sendMessage(text).catch(() => {});
