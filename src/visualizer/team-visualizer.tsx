 ...
 *
 * Watches ~/.tinysdlc/events/ for structured JSON events emitted by the
    ...
    return (
        <Box flexDirection="column" marginBottom={1}>
            <Box>
                ...
                <Text bold color="white">TinySDLC Team Visualizer</Text>
...
function App({ filterTeamId }: { filterTeamId: string | null }) {
    ...
    const handleEvent = useCallback((event: TinySDLCEvent) => {
        switch (event.type) {
        ...
        try {
            ...
            for (const file of existing) {
                try {
                    ...
                    const event: TinySDLCEvent = JSON.parse(content.trim());
    ...
    // Poll queue depth
    useEffect(() => {
        const queueIncoming = path.join(TINYSDLC_HOME, 'queue/incoming');
