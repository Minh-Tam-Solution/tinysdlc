...
/**
 * Ensure agent directory exists with template files copied from TINYSDLC_HOME.
 * Creates directory if it doesn't exist and copies .claude/, heartbeat.md, and AGENTS.md.
 * If agent has an sdlc_role and a matching template exists, uses the role-specific AGENTS.md.
 */
export function ensureAgentDirectory(agentDir: string, agent?: AgentConfig, force = false): void {
    if (fs.existsSync(agentDir) && !force) {
        return; // Directory already exists — skip unless reinit requested
    }
    ...
    // Copy AGENTS.md — prefer SDLC role-specific template if available
    let sourceAgents = path.join(SCRIPT_DIR, 'AGENTS.md');
    if (agent?.sdlc_role) {
        const roleTemplate = path.join(SCRIPT_DIR, 'templates', 'agents', agent.sdlc_role, 'AGENTS.md');
        if (fs.existsSync(roleTemplate)) {
            sourceAgents = roleTemplate;
        }
    }
    ...
    // Copy AGENTS.md as .claude/CLAUDE.md (uses same source — role-specific if available)
    if (fs.existsSync(sourceAgents)) {
    ...
    const targetTinysdlc = path.join(agentDir, '.tinysdlc');
    fs.mkdirSync(targetTinysdlc, { recursive: true });
