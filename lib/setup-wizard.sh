#!/usr/bin/env bash
# TinySDLC Setup Wizard

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SETTINGS_FILE="$HOME/.tinysdlc/settings.json"
...
echo ""
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}  TinySDLC - Setup Wizard${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
if [[ "$SETUP_AGENTS" =~ ^[yY] ]]; then
    ...
    while [ "$ADDING_AGENTS" = true ]; do
        ...
        if [ "$NEW_PROVIDER" = "anthropic" ]; then
            ...
            echo "  Model: 1) GPT-5.1  2) GPT-5.2  3) GPT-5.3 Codex"
            read -rp "  Choose [1-3, default: 1]: " NEW_MODEL_CHOICE
            case "$NEW_MODEL_CHOICE" in
                2) NEW_MODEL="gpt-5.2" ;;
                3) NEW_MODEL="gpt-5.3-codex" ;;
                *) NEW_MODEL="gpt-5.1" ;;
            esac
        fi
...
if [ -d "$PROJECT_ROOT/.claude" ]; then
    cp -r "$PROJECT_ROOT/.claude" "$TINYSDLC_HOME/"
fi
if [ -f "$PROJECT_ROOT/heartbeat.md" ]; then
    cp "$PROJECT_ROOT/heartbeat.md" "$TINYSDLC_HOME/"
fi
...
if [ -d "$TINYSDLC_HOME/.claude" ]; then
    cp -r "$TINYSDLC_HOME/.claude" "$DEFAULT_AGENT_DIR/"
fi
if [ -f "$TINYSDLC_HOME/heartbeat.md" ]; then
    cp "$TINYSDLC_HOME/heartbeat.md" "$DEFAULT_AGENT_DIR/"
fi
if [ -f "$TINYSDLC_HOME/AGENTS.md" ]; then
    cp "$TINYSDLC_HOME/AGENTS.md" "$DEFAULT_AGENT_DIR/"
fi
echo -e "${GREEN}✓ Created default agent directory: $DEFAULT_AGENT_DIR${NC}"

# Create ~/.tinysdlc/files directory for file exchange
mkdir -p "$TINYSDLC_HOME/files"
echo -e "${GREEN}✓ Created files directory: $TINYSDLC_HOME/files${NC}"
...
for agent_id in "${ADDITIONAL_AGENTS[@]}"; do
    ...
    mkdir -p "$AGENT_DIR"
    if [ -d "$TINYSDLC_HOME/.claude" ]; then
        cp -r "$TINYSDLC_HOME/.claude" "$AGENT_DIR/"
    fi
    if [ -f "$TINYSDLC_HOME/heartbeat.md" ]; then
        cp "$TINYSDLC_HOME/heartbeat.md" "$AGENT_DIR/"
    fi
    if [ -f "$TINYSDLC_HOME/AGENTS.md" ]; then
        cp "$TINYSDLC_HOME/AGENTS.md" "$AGENT_DIR/"
    fi
