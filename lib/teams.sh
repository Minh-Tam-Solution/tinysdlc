#!/usr/bin/env bash
# Team management functions for TinySDLC
...
update_agent_team_info() {
    ...
    if grep -q '<!-- TINYSDLC_TEAM_START -->' "$agents_md" 2>/dev/null; then
        sed -i.bak '/<!-- TINYSDLC_TEAM_START -->/,/<!-- TINYSDLC_TEAM_END -->/d' "$agents_md"
        rm -f "$agents_md.bak"
    fi
