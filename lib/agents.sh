#!/usr/bin/env bash
# Agent management functions for TinySDLC
...
ensure_agent_skills_links() {
    ...
    if [ ! -d "$skills_src" ]; then
        skills_src="$TINYSDLC_HOME/.agents/skills"
    fi
    ...
    case "$provider_arg" in
        anthropic)
            if [ -n "$model_arg" ]; then
                ...
                echo ""
                echo "Use 'tinysdlc agent provider ${agent_id} anthropic --model {sonnet|opus}' to also set the model."
            fi
            ;;
        openai)
            if [ -n "$model_arg" ]; then
                ...
                echo ""
                echo "Use 'tinysdlc agent provider ${agent_id} openai --model {gpt-5.3-codex|gpt-5.2}' to also set the model."
            fi
            ;;
        *)
            echo "Usage: tinysdlc agent provider <agent_id> {anthropic|openai} [--model MODEL_NAME]"
            echo ""
            echo "Examples:"
            echo "  tinysdlc agent provider coder                                    # Show current provider/model"
            echo "  tinysdlc agent provider coder anthropic                           # Switch to Anthropic"
            echo "  tinysdlc agent provider coder openai                              # Switch to OpenAI"
            echo "  tinysdlc agent provider coder anthropic --model opus              # Switch to Anthropic Opus"
            echo "  tinysdlc agent provider coder openai --model gpt-5.3-codex        # Switch to OpenAI GPT-5.3 Codex"
            exit 1
            ;;
    esac
