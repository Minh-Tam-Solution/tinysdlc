#!/usr/bin/env bash
# Messaging and logging functions for TinySDLC
...
channels_reset() {
    ...
    if [ "$ch" = "whatsapp" ]; then
        rm -rf "$SCRIPT_DIR/.tinysdlc/whatsapp-session"
        rm -f "$SCRIPT_DIR/.tinysdlc/channels/whatsapp_ready"
        rm -f "$SCRIPT_DIR/.tinysdlc/channels/whatsapp_qr.txt"
        rm -rf "$SCRIPT_DIR/.wwebjs_cache"
        echo -e "${GREEN}✓ WhatsApp session cleared${NC}"
        echo ""
        echo "Restart TinySDLC to re-authenticate:"
    ...
    if [ -n "$token_key" ]; then
        ...
        echo -e "  ${GREEN}tinysdlc setup${NC}"
        echo ""
        echo "Or manually edit .tinysdlc/settings.json to change ${token_key}"
    fi
}
