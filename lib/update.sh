...
check_for_updates() {
    ...
    # Skip if disabled
    if [ "${TINYSDLC_SKIP_UPDATE_CHECK:-}" = "1" ]; then
        return 1
    fi
...
show_update_notification() {
    ...
    echo ""
    echo -e "  Update:  ${GREEN}tinysdlc update${NC}"
...
# Perform update
do_update() {
    echo -e "${BLUE}TinySDLC Update${NC}"
    echo "==============="
    echo ""

    # Check if running
    if session_exists; then
        echo -e "${YELLOW}Warning: TinySDLC is currently running${NC}"
        echo ""
