#!/usr/bin/env bash
# Daemon lifecycle management for TinySDLC
...
start_daemon() {
    ...
    if [ $load_rc -eq 2 ]; then
        ...
        if [ $load_rc -ne 0 ]; then
            ...
            echo "  Or reconfigure: tinysdlc setup"
            return 1
        fi
    ...
    if [ ${#ACTIVE_CHANNELS[@]} -eq 0 ]; then
        echo -e "${RED}No channels configured. Run 'tinysdlc setup' to reconfigure${NC}"
        return 1
    fi
    ...
    for ch in "${ACTIVE_CHANNELS[@]}"; do
        ...
        if [ -n "$token_key" ] && [ -z "${CHANNEL_TOKENS[$ch]:-}" ]; then
            ...
            echo "Run 'tinysdlc setup' to reconfigure"
            return 1
        fi
    done

    # Write tokens to .env for the Node.js clients
    # SEC-009: restrict to owner read/write only — prevents token theft on shared systems
    ...
    tmux new-session -d -s "$TMUX_SESSION" -n "tinysdlc" -c "$SCRIPT_DIR"
    ...
    echo ""
    echo -e "${GREEN}✓ TinySDLC started${NC}"
    echo ""
    ...
    if [ "$whatsapp_pane" -ge 0 ]; then
        ...
        QR_FILE="$TINYSDLC_HOME/channels/whatsapp_qr.txt"
        READY_FILE="$TINYSDLC_HOME/channels/whatsapp_ready"
        QR_DISPLAYED=false
        ...
        if [ $i -eq 60 ] && [ ! -f "$READY_FILE" ]; then
            ...
            echo ""
            echo -e "${YELLOW}Try restarting TinySDLC:${NC}"
            echo -e "  ${GREEN}tinysdlc restart${NC}"
            echo ""
            echo "Or check WhatsApp client status:"
            echo -e "  ${GREEN}tmux attach -t $TMUX_SESSION${NC}"
            echo ""
            echo "Or check logs:"
            echo -e "  ${GREEN}tinysdlc logs whatsapp${NC}"
            echo ""
        fi
    fi
...
stop_daemon() {
    ...
    echo -e "${GREEN}✓ TinySDLC stopped${NC}"
    log "Daemon stopped"
}

# Restart daemon safely even when called from inside TinySDLC's tmux session
restart_daemon() {
...
status_daemon() {
    ...
    if session_exists; then
        ...
        echo "  Start: tinysdlc start"
    fi

    echo ""

    # Channel process status
    local ready_file="$TINYSDLC_HOME/channels/whatsapp_ready"
